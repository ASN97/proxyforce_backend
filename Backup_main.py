...
def generate_timeline_from_prompt(project_id: str, prompt: str):
    # Extract or simulate milestones from the prompt
    milestones = [
        {
            "id": 1,
            "name": "Design Phase",
    ...
    with open(PROJECTS_FILE, "r+") as f:
        data = json.load(f)
        project = data["projects"].get(str(project_id))
        if project:
            project["milestones"] = milestones
            f.seek(0)
            json.dump(data, f, indent=2)
            f.truncate()
    return "Timeline updated with 2 milestones."

def assign_tasks_from_prompt(project_id: str, prompt: str):
    # Simulate basic tasks
    tasks = [
        {"name": "Set up frontend", "assignee": "Alice", "deadline": (datetime.now() + timedelta(days=5)).isoformat(), "priority": "high", "status": "not-started"},
        {"name": "Connect backend", "assignee": "Bob", "deadline": (datetime.now() + timedelta(days=5)).isoformat(), "priority": "high", "status": "not-started"},
    ]
    with open(PROJECTS_FILE, "r+") as f:
        data = json.load(f)
        project = data["projects"].get(str(project_id))
        if project:
            project.setdefault("tasks", []).extend(tasks)
            f.seek(0)
            json.dump(data, f, indent=2)
            f.truncate()
    return "Tasks assigned to Alice and Bob."
...
class ChatRequest(BaseModel):
    message: str
    role: str  # No default; required from frontend
    tier: str  # No default; required from frontend

import os
openai.api_key = os.getenv("OPENAI_API_KEY")

def get_project_context(project_id: str, role: str, tier: str) -> str:
    if not os.path.exists(PROJECTS_FILE):
    ...
    role_title = role_titles.get(role.lower(), "Team Assistant")
    experience = experience_levels.get(tier, "3 years")

    context = f"""You are a {role_title} AI with {experience} of experience.
You are supporting the following project:
...
"""
    ...
    objectives = project.get("additional_info", {}).get("objectives", "N/A")
    success = project.get("additional_info", {}).get("successCriteria", "N/A")

    context += f"\nObjectives: {objectives}\nSuccess Criteria: {success}\n"
    context += f"Respond as a helpful and concise {role_title}, using this project data."
    return context
...
async def send_chat_message(project_id: str, chat: ChatRequest = Body(...)):
    try:
        system_prompt = get_project_context(project_id, chat.role, chat.tier)
